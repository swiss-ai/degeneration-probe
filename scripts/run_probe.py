...
Checks whether a trained probe checkpoint exists at
  ...
  - FOUND   → run evaluation via degeneration_probe/evaluation/evaluate.py internals
              and log metrics to W&B so the analysis notebook picks them up automatically.
  - NOT FOUND → delegate to scripts/train_probe.py via Hydra (full training + eval).
