...
```bash
export HF_HOME=/capstor/scratch/cscs/$USER/degeneration-probe/hf_cache
export HF_HUB_CACHE=/capstor/scratch/cscs/$USER/degeneration-probe/hf_cache
export UV_CACHE_DIR=/capstor/scratch/cscs/$USER/degeneration-probe/uv_cache
export WANDB_DIR=/capstor/scratch/cscs/$USER/degeneration-probe/wandb
```
...
```bash
find /capstor/scratch/cscs/$USER/degeneration-probe -exec touch {} +
```
...
srun --account=infra01 --partition=debug --time=00:30:00 \
    --environment=/iopsstor/scratch/cscs/$USER/degeneration-probe/cluster/env.toml \
    --pty bash
